...
**PR #94 `fix/types-baseline` — eliminate 447 tsc baseline errors**: Brings the baseline from 447 → 0 with no behavioral changes. Mostly mechanical (missing type annotations, optional-chain narrowing, `useState` initializers, supabase RPC result-type unwrapping). Surfaced one real bug — `MultiFamilyEVInput.evChargersPerUnit` was a stale field that Vite's transpile-only build had silently shipped past the broken type checker. With 0 errors as the floor, the next PR can gate the build on tsc.

**PR #95 `ci/add-tsc-gate` — CI workflow with tsc + build + tests**: First GitHub Actions workflow in the repo. Runs `npx tsc --noEmit` → `npm run build` → `npm test` on every push / PR to main. The tsc gate is the load-bearing addition — Vite skips type validation, so without this gate the 0-error baseline drifts back up unchecked. Placeholder Supabase env vars at job scope satisfy `lib/supabase.ts`'s module-load throw without committing real secrets (VITE_* vars are public by design; tests use mocks).

**PR #96 `chore/regen-database-types` — regenerate database.types.ts from Supabase MCP**: Replaces PR #94's hand-patched dedup with a fresh `mcp__plugin_supabase_supabase__generate_typescript_types` regen. 31-line diff vs hand-patched proves the dedup was structurally correct (no missing tables / fields). Surfaced a type-safety regression: Postgres CHECK constraints aren't introspected by gen-types, so `cover_mode` widened from a 3-literal union to `string` — fixed inline with explicit `as CoverMode` casts at the two call sites where the CHECK guarantees runtime safety. The deeper lesson for future schema work: enums > CHECK constraints when narrow-typed columns matter.

**PR #97 `fix/panel-pdf-demand-display` — first-pass demand surface (aggregator + EVEMS + dwelling consolidation)**: Threads the same `calculateAggregatedLoad` result the in-app Panel Summary uses into the PDF renderer. Three Load Summary modes by panel type: (1) dwelling unit panels (NEC 220.82/.83) — 5-column unified card merges the former "Dwelling Demand Breakdown" card INTO the Load Summary as `Total | Demand | Demand Amps | General@Tiered | Climate@100%` so 42-circuit dwelling panels stop overflowing to a second page; (2) aggregator/EVEMS-clamped panels (NEC 220.84 or 625.42) — 4-column card surfaces `Total | Connected | Demand kVA | Demand Amps` with the dominant NEC article annotated in the title (e.g. a 1000A MDP bus that summed to 1380A nameplate now shows 999A demand explicitly); (3) default (panel-local, no factor reduction) — unchanged. Separate EVEMS Aggregate Setpoint card suppressed when its values now appear inline. Per-panel `calculateAggregatedLoad` wired through `permitPacketGenerator.tsx` with `multiFamilyContext` passed ONLY on the MDP (the 220.84 factor applies at the aggregate, not chained through hierarchy).

**PR #98 `fix/panel-pdf-commercial-demand` — demand on every panel + NEC 220 audit breakdown + layout tightening**: Two layered changes. **First**: loosens PR #97's trigger from `hasMeaningfulAggregated` (only fired when demand < connected) to `hasAggregatedDemand` (fires on any positive aggregator result). All non-dwelling panels — including commercial sub-panels with no downstream feeders and no EVEMS clamp — now show Demand kVA + Demand Amps inline. For panels where demand == connected (all loads at 100% factor), both columns show the same value and the inspector sees the NEC reference that was applied. **Second**: adds a compact 5-column breakdown table below the Load Summary card sourced from `aggregatedLoad.demandBreakdown[]` — `Load Type | Connected | Demand | Factor | NEC`. Lets AHJ reviewers verify how the aggregate demand was derived (Receptacles @ 220.44, Motors @ 220.50, HVAC @ 220.60, etc.) without flipping to the separate Load Calculation Summary page. **Third (folded in during review)**: tightened panel-schedule layout by ~80pt total to fit the new breakdown table on 42-circuit panels — `tableRow.paddingVertical 4→2.5` is the load-bearing change (31.5pt saved across 21 rows), with header/info-grid spacing also recovered. The per-row multiplier is the key insight: fixed-cost header savings alone wouldn't have been enough.

**PR #99 `fix/panel-pdf-dwelling-breakdown` — NEC 220.82/.83 audit table on dwelling panels with construction-type routing**: Brings the audit-trail treatment from PR #98 to residential dwelling panels (single-family MDPs AND multi-family unit panels) with the correct NEC method routed by construction context. Title resolves dynamically: `Load Summary (NEC 220.83 — Existing Dwelling)` when `showExistingNewMarkers` is true vs `Load Summary (NEC 220.82 — Optional Method)` when false. The breakdown table renders the tier-split explicitly — `General loads (first 10 kVA @ 100%)` + `General loads (remainder @ 40%)` for panels above the tier threshold, collapsed to a single `(under tier threshold)` row otherwise, plus a `Climate (largest of HVAC, non-coincident)` row at `220.60` when at least one of heating/cooling is present. Reuses the existing `showExistingNewMarkers` prop as the 220.82/.83 router — no new prop with overlapping semantics. Multi-family building MDPs (NEC 220.84) continue through the aggregator path unchanged — that already had its own per-unit + house + EV breakdown from PR #98.

**Inspector ergonomics shift worth flagging**: Before this arc, an AHJ reviewer looking at a panel page had to flip to the Load Calculation Summary, match the panel to a row, verify the demand factor application, and flip back to confirm bus rating compatibility. After this arc, those steps collapse to a single glance at the bottom of the panel page. The time-to-permit-approval savings aren't visible in any single screenshot but accumulate across the AHJ's review queue. **DB schema additions**: none — pure rendering + orchestration changes. **Test count**: 995/995 passing throughout. **Visual proofs**: `example_reports/PanelSchedule_Dwelling_Existing_22083.pdf` + `PanelSchedule_Dwelling_New_22082.pdf` regeneratable via `E2E_DUMP_PDFS=1 npm test`; full multifamily packet at `Permit_Packet_MF_EV_Existing_2026-05-17.pdf` shows the aggregator card + breakdown table on the MDP + sub-panel pages.

---

## 2026-05-17: Sprint 2C M3 — Existing/New construction end-to-end + permit-packet polish (PRs #79, #80, #81 stacked)

Three stacked PRs close the **Existing vs New construction differentiation gap** the user surfaced as a multi-scenario blocker: contractors couldn't express "existing MF building + new EV infrastructure", "existing commercial + new sub-panel via NEC 220.87 Method 2", or "existing commercial + new loads with utility-bill 220.87 Method 1" in a way the rendered permit packet reflected end-to-end. PR #78 (the predecessor — superseded) had removed the broken inline MF-EV form from the Permit Packet Generator without restoring the proper Tools Hub → packet wiring. PR #79 fixes that the right way; PR #80 extends the same pattern up one equipment level to panels; PR #81 polishes the result based on user review feedback.

**PR #79 `feat/existing-new-construction-flow` — Tools-Hub-as-source-of-truth MF-EV wiring + Existing/New foundation**: Replaces the broken inline duplicate form (passing a stale `evChargersPerUnit` shape that triggered TS2353 — Vite transpile-only build silently shipped junk data when the toggle was used) with a clean source-of-truth model. `MultiFamilyEVCalculator` now persists its full `result` to `project.settings.residential.mfEvCalculation` (debounced 750ms); `PermitPacketGenerator` reads it directly. Section toggle auto-disables when `dwellingType !== 'multi_family'` OR no result is persisted, with a CTA pointing at the Tools Hub. Adds `service_modification_type: 'service-upgrade'` as a top-level Project Status option (was buried in the exemption-screening sub-panel only). `nec22087Narrative` inputs now persist on `ProjectSettings` and survive reload. Building-type-aware help text on the Project Status toggle (Commercial sees "Method 1 utility data or Method 2 calculated from panel schedule" instead of dwelling-only 220.82/220.83 copy). `PanelSchedule` "Add Circuit" auto-defaults to `is_proposed: true` in existing-construction mode (closes the real EXIST/NEW gap — previously every manually-added circuit silently buried as existing). `ServiceUpgradeWizard` "Save to Packet 220.87 Narrative" button persists Quick Check inputs into the packet's narrative section with amps→kVA conversion. 5 new vitest tests in `multiFamilyEVPersistenceWiring.test.ts` pin the Tools Hub → packet data-shape contract.

**PR #79 follow-on (commit `98f0cbb`) — 4 renderer bugs from Method 1 PDF review**: User caught (1) "MIN REQUIRED AIC: 22100kA" — `fmtAmps` was missing on the AIC card so raw amps rendered with a "kA" hardcoded suffix; (2) MDP panel schedule showing the H1 feeder twice — `synthesizeFeederCircuits` wasn't deduping against explicit feeder circuits at the parent's matching slot; (3) no EXIST/NEW differentiation visible because the test fixture had zero `is_proposed: true` circuits; (4) 220.87 narrative cards showed kVA only on 3 of 4 cards — added a `kVAToAmps` inverse helper and the demand cards now show amps in the subscript ("245.0 kVA / 295 A • measured (no 125% mult.)").

**PR #80 `feat/panel-level-is-proposed` — equipment-level is_proposed, new-panel UX end-to-end**: PR #79's circuit-level pattern lifted one equipment level. New migration `20260517_panel_is_proposed.sql` adds `panels.is_proposed boolean NOT NULL DEFAULT false`. `OneLineDiagram` Add Panel modal exposes a "This is a new (proposed) panel" checkbox (visible only on existing-construction projects for non-MDP panels; auto-defaults to TRUE — most common case for sub-panels added to an existing service). Renderer: dashed amber border + corner "NEW" badge on proposed panel nodes in both the riser PDF (`PermitPacketDocuments.tsx::RiserDiagram`) AND the in-app one-line (`OneLineDiagram.tsx::renderPanelGlyph` — pixel-consistent treatment). Panel-schedule cover shows a "PROPOSED — NEW INSTALLATION" amber banner at the top of any proposed panel's cover sheet, both render paths. Commercial test fixture restructured to demonstrate the full story end-to-end: existing MDP + existing H1, NEW EVSE-1 sub-panel fed from H1 with the 4 EVSE circuits inside, + a NEW "Feed to EVSE-1" feeder circuit on H1.

**PR #81 `feat/voltage-persistence-legend-polish` — voltage selector fix + form-field persistence + legend polish**: Three user-flagged bugs after testing PR #80: (1) panels created via Add Panel modal showed "480V / 120V" — both branches of the L-N display ternary hardcoded `'120V'` (correct value for 480V 3Ø wye is 277V). Plus the modal exposed standalone voltages (120/208/240/277/480) and a separate phase selector, letting users pick electrically-invalid combos like 480V single-phase. Replaced both with a single system-type selector matching `ProjectSetup.tsx`'s 3-option pattern (240/120 1Ø, 208/120 3Ø, 480/277 3Ø). New shared `lib/electricalDisplay.ts` with `lineToNeutralVoltage(v, p)`, `formatPanelVoltageDisplay`, `SYSTEM_TYPE_OPTIONS`, `systemTypeFromVP` / `vpFromSystemType`. (2) Permit Packet form fields (preparedBy, contractor license, scope of work, service type, meter location, conductor routing) now persist to `project.settings.permitPacketDefaults` (debounced 750ms hydrate-then-persist pattern mirroring nec22087Narrative). (3) Panel schedule legend: "* = Proposed new circuit (added under this permit)" → "* = Proposed new circuit"; proposed-circuit descriptions now render in **Helvetica-Bold** so new circuits visually pop on a dense 30-42-row schedule.

**End-to-end proof PDFs** generated via the existing `generatePermitPacket` pipeline and copied to `/home/augusto/Obsidian Notes/Projects/Sparkplan Test Packets/`:
- `Permit_Packet_MF_EV_Existing_2026-05-17.pdf` (100.5 KB) — Scenario 1: Existing MF + EV
- `Permit_Packet_Commercial_Method2_Calculated_2026-05-17.pdf` (112 KB) — Scenario 2: Commercial Existing + calculated × 1.25
- `Permit_Packet_Commercial_Method1_UtilityBill_2026-05-17.pdf` (112 KB) — Scenario 3: Commercial Existing + 12-month utility bill (no 1.25× per NEC 220.87(A))

**DB schema additions**: One real migration (`20260517_panel_is_proposed.sql`) — `panels.is_proposed boolean NOT NULL DEFAULT false`. JSONB extensions to `projects.settings` (no migration): `permitPacketDefaults`, `nec22087Narrative`, `residential.mfEvCalculation`. **Migration must be run manually in the Supabase SQL Editor** per CLAUDE.md convention; the auto-defaults to false mean existing projects render unchanged.

**Test count**: 995/995 passing across 52 files (up from 988 baseline). Build green; PermitPacketGenerator chunk: 680.75kB → 677.44kB (smaller because the broken inline MF-EV form was deleted).

---

## 2026-05-16: Sprint 2C M2 — AHJ coverage expansion + manifest reuse flexibility (PRs #70–#75 in-flight)

Six parallel PRs land Sprint 2C M2 — 4 new AHJ manifests (Tier 1 from the coverage audit) plus 3 UX flexibility gaps so contractors in unmodeled FL cities can reuse an existing manifest without misrepresenting their AHJ. AHJ count: **5 → 9 registered**, FL metro population served: **~2.3M → ~3.5M directly**, and every other FL city unblocked via the new manifest-template dropdown.

**PR #70 `feat/orlando-manifest-requirements` — Orlando manifest populate**: Backports Sprint 2C M1's requirements[] schema onto the Orlando manifest, which PR #56 missed (PR #51 had shipped it as a scaffold with `requirements: []` and the comment "Sprint 2C M1 populates this"; PR #56 populated the other 4 AHJs but skipped Orlando). 17 entries sourced line-by-line from `docs/AHJ_COMPLIANCE_AUDIT_2026-05-04.md`'s Orlando matrix: 1 application (NOC > $5k), 6 plans (one-line/AIC, load calc, revised panel, AFC for new-service path, grounding for new-service path, site plan), 3 uploads (EVSE cut sheets UL-2202/2594, fire stopping, HVHZ anchoring), 5 narratives (code basis, VD note, contractor license, NEC 220.87 narrative for existing-service path, PE seal for pe_required lane), 2 inspections (electrical rough, electrical final). Path-dependent requirements reuse the same `scope` / `lane` predicates the manifest's `sectionPredicates` already encoded. PE seal uses `detect: () => false` until Sprint 3 (C5) lands PAdES infrastructure. 28 new tests in `tests/orlandoManifest.test.ts`; updated 2 pre-existing scaffold tests (`orlandoManifestE2E.test.ts:85`, `checklistEngine.test.ts:116`) that locked in `requirements: []`.

**PR #71 `feat/manifest-flexibility-gaps` — 3 UX flexibility gaps**: A contractor in Sanford / Winter Park / Lake Mary / Cape Coral can now reuse Orlando's manifest as a template without their cover sheet saying "City of Orlando". Three gaps closed: **(Gap 1)** `projects.settings.manifest_template_id` separation from `jurisdiction_id` — a new dropdown in the UI populated from `ALL_MANIFESTS` overrides the jurisdiction-derived lookup; AHJ name still comes from the actual jurisdiction field. **(Gap 2)** Artifact-slot toggle UI mirrors the existing section-toggle grid in `PermitPacketGenerator.tsx:927-967`; writes through to `section_overrides.artifactTypes.{key}` which `applyUserOverrides` already consumed. Closes a TODO from PR #51. **(Gap 3)** Per-project string overrides for `general_notes_override` / `code_references_override` / `sheet_id_prefix_override` in `projects.settings` (jsonb extension, no migration). Resolution chain mirrors existing `necEdition` pattern: `data.X ?? data.XOverride ?? data.manifest?.X`. UI adds a collapsible "Override AHJ details" panel. 19 new tests. Backward compat preserved — projects without any override fields behave IDENTICALLY to today.

**PR #72 `feat/ahj-city-of-miami` — City of Miami manifest**: Biggest single-AHJ gap in FL (~440k pop). Independent of Miami-Dade County RER (already covered) — the County only handles unincorporated areas. **HVHZ ON** (all of Miami-Dade is HVHZ per FBC 8th ed Ch. 16, 175 mph 3-sec gust). NEC 2020 uniform across building types (no MD-RER-style H34 per-buildingType PRG fork). Sheet prefix `E-` (no `EL-` mandate at City level). **17 requirements / 44 tests / MEDIUM confidence** — City of Miami publishes no dedicated EV-charging checklist (verified by direct fetch attempts to miami.gov), so manifest composed from page metadata + permit-aggregator corroboration + FL statute baselines. Registry includes a regression-guard test that "Miami-Dade County" jurisdiction strings do NOT route to `city-of-miami` (substring collision risk on "miami").

**PR #73 `feat/ahj-orange-county` — Orange County (unincorporated) manifest**: Closes Orlando-metro gap. Covers Pine Hills, Doctor Phillips, Orlovista, Winter Garden suburbs (~330k of OC's 1.5M). Distinct from City of Orlando (independent AHJ, already covered). Subjurisdiction `'unincorporated'` mirrors miami-dade.ts pattern. **Notable divergence: NOC threshold = $2,500** (lower than FL Statute 713.13's $5k statewide default; verbatim from OC Electrical Permit page). NOC predicate defaults ON for ALL building types (vs Orlando/MD where it's non-SFR only). NOT in HVHZ (Central FL inland). `hvhz_anchoring` predicate-gated to commercial/multi-family outdoor installs only. 15 reqs / 33 tests / MEDIUM confidence (HIGH on code basis + form 23-30 verbatim via pdftotext; MEDIUM on EV-specific scope — no dedicated OC EV checklist).

**PR #74 `feat/ahj-st-petersburg` — City of St. Petersburg manifest**: Closes Tampa Bay metro gap. ~265k pop, second-largest Tampa Bay AHJ. Pinellas does NOT roll up to the hillsborough-tampa joint manifest. **HVHZ verdict: OUT** (only MD + Broward + Monroe are HVHZ proper) **but Pinellas IS in Wind-Borne Debris Region at 140 mph ultimate design wind speed** — anchoring uses FL Product Approval path (NOA optional). `hvhz_anchoring` slot kept ON for non-SFR per Sprint 2C H19 statewide convention. 24 reqs / 51 tests / MEDIUM confidence (St. Pete also publishes no dedicated EV checklist).

**PR #75 `feat/ahj-jacksonville-duval` — Jacksonville/Duval consolidated manifest**: Largest FL city not previously covered (~920k pop). First North-FL AHJ added. **Consolidated city-county since 1968** — one government, one AHJ for both city and county functions (modeled as `jurisdictionType: 'city'`). Four retained "Beaches" munis (Jax Beach / Atlantic Beach / Neptune Beach / Baldwin) flagged as out-of-scope independent AHJs for follow-up. NOT in HVHZ (North FL); `hvhz_anchoring` explicitly NOT in `relevantArtifactTypes` (cleaner than gating with always-false predicate). 25 reqs / 44 tests / MEDIUM-HIGH confidence.

**Total**: 6 PRs / 9 manifest files / 5 test files / 4 docs / ~3000 LOC added. Net test count gain: ~191 across all 6 PRs (28 + 44 + 33 + 51 + 44 + 19 minus pre-existing scaffold updates). Build green and full suite at 960/960 passing in local pre-merge verification. **DB schema additions** (jsonb extensions to `projects.settings`, no migration files needed): `manifest_template_id?: string`, `general_notes_override?: string[]`, `code_references_override?: string[]`, `sheet_id_prefix_override?: 'E-' | 'EL-' | 'ES-'`.

**Sequential-merge gotcha worth carrying forward**: my local sequential merges of the 5 agent branches surfaced 4 mechanical `registry.ts` conflicts. One of them resolved cleanly in the second-to-last merge but left an orphan `<<<<<<< HEAD` marker from the previous resolution — Git accepts commits containing literal `<<<<<<<` text once the file is `git add`-ed (only blocks unmerged paths, not conflict-marker patterns), so the orphan only surfaced at `npm run build` time via esbuild's `Transform failed: Expected identifier but found "<<"`. Fixed forward with a 1-line cleanup commit. A pre-commit hook (`grep -E '^(<<<<<<<|=======|>>>>>>>)' --include='*.{ts,tsx,js,jsx}'`) would have caught it.
