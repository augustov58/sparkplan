...
## 2026-05-06: AHJ Compliance Audit — Per-EVSE Branch Math + EVEMS Feeder Reduction (C4)

**User-Facing Bug Fixes:**
- **EV sub-panel schedule pages on the permit-packet PDF now show correct per-EVSE branch circuit loads.** Page 14 of the audit packet was showing each Level-2 (48A) EV charger branch circuit at 3,996 VA — well below the NEC 220.57(A) minimum and well below the 11,520 VA continuous-nameplate value those branch conductors actually carry. Each branch row now shows 11,520 VA (or the NEC 220.57(A) 7,200 VA minimum, whichever is larger), regardless of whether the panel is EVEMS-managed. This matches what an AHJ reviewer expects and what NEC 625.40 + 210.19 require for branch conductor sizing.
- **EVEMS load reduction now correctly applied at the feeder/service level only (NEC 625.42), not at branch circuits.** Pre-fix, the EVEMS-managed per-charger setpoint share was persisted into each branch circuit's `load_watts` field at autogeneration time. NEC 625.42 only allows EVEMS to reduce the FEEDER and SERVICE; branch conductors must still handle the full continuous nameplate. Now the autogeneration writes nameplate-faithful branch values, and the feeder/service NEC 625.42 reduction is applied downstream by `calculateAggregatedLoad` clamping `totalDemandVA` to the calculator's exact EVEMS setpoint (carried via a metadata-only "EVEMS Aggregate Setpoint (NEC 625.42)" marker circuit emitted by the autogen).
- **In-app EV Sub-Panel summary now shows the EVEMS-clamped Demand Amps.** Previously the EV Sub-Panel "Demand Amps" tile showed the raw nameplate sum (e.g. 585.6 A on a 400A panel — visibly wrong), because the panel-summary code only switched to the aggregated-load view for panels feeding downstream sub-panels. Now any panel where the EVEMS clamp engages shows the clamped value (e.g. ~200 A on the audit fixture's EV Sub-Panel) with a small "(NEC 625.42 EVEMS)" annotation. The MDP "Demand Amps" likewise drops from ~1199 A (over the 1000A breaker) to ~999 A (within the breaker) once the explicit setpoint replaces the over-generous panel-breaker proxy.
- **EVEMS aggregate setpoint is now visible in the EV Sub-Panel schedule (PDF + UI).** The metadata row that previously read "EVEMS Load Management System  500 VA" (a placeholder the audit itself flagged as misleadingly low) now reads "EVEMS Aggregate Setpoint (NEC 625.42)  ~48 kVA" (the calculator's actual managed maximum demand for the fixture). AHJ reviewers can verify the NEC 625.42 setpoint at a glance from the panel schedule.

**Why this matters for the FL pilot:** C4 was the last engine-correctness bug in Sprint 1. With C4 shipped, all calculated values on the audit packet — load calc page (C1), voltage drop page (C2), per-EVSE branch loads (C4), and panel-schedule phase columns (C6) — match what an electrical engineer or AHJ reviewer expects to see. Combined with C3's advisory metadata + B-1's AI live-derive, the packet is no longer producing demonstrably wrong arithmetic on any of its calculation-bearing pages. Remaining work is Sprint 2 (FL Permit Mode v1 — TOC, sheet IDs, jurisdiction-checklist engine) and Sprint 3 (PE seal workflow).

**Technical (three-pronged fix; the second prong was added during user review of the in-app panel summary, after the first commit on this branch shipped a panel-breaker proxy that over-clamped by ~2x):**
- **Branch row nameplate (NEC 220.57(A)).** `data/ev-panel-templates.ts`: `circuitLoadVA = Math.max(7200, V × I)` regardless of `useEVEMS`. The deprecated `evemsLoadVAPerCharger` field is preserved for backward compat but ignored. `services/autogeneration/multiFamilyProjectGenerator.ts:generateEVPanel` no longer plumbs the override.
- **Explicit EVEMS setpoint marker (NEC 625.42).** `data/ev-panel-templates.ts:CustomEVPanelConfig` gained `evemsSetpointVA?: number`. When provided alongside `useEVEMS=true`, the template emits a metadata-only circuit with description `"EVEMS Aggregate Setpoint (NEC 625.42)"` and `loadVA = setpoint`. The autogen computes the value as `Math.round(scenario.powerPerCharger_kW × 1000 × chargerCount)` — the multi-family EV calculator's exact `actualKWPerChargerWithEVEMS × numChargers` from the `withEVEMS` scenario (e.g. ~48 kVA for the audit fixture). Legacy projects without the marker fall back to a `panel.main_breaker_amps × voltage` upper-bound proxy with a `[NEC 625.42 EVEMS]` warning attached to the breakdown.
- **Feeder/service-level clamp inside `calculateAggregatedLoad`.** `services/calculations/upstreamLoadAggregation.ts` gained file-private helpers `isEVEMSManagedPanel`, `isEVEMSMarkerCircuit`, `isEVEMSExplicitSetpointMarker`, and `evemsSetpointVA(panel, circuits)` (reads from the explicit marker first, falls back to the legacy proxy). `collectCircuitLoads` + the `directConnected` reducer in `collectAllDownstreamLoads` skip marker circuits — they're metadata, not real loads, and would otherwise double-count. `calculateAggregatedLoad` clamps `totalDemandVA` to the setpoint when applicable on both the standard NEC 220 path and the NEC 220.84 multi-family path, pushing a breakdown row that explains the clamp. `MultiFamilyContext` gained an `evDemandVA?` field so the MDP-level NEC 220.84 calc subtracts raw connected EV from the dwelling base while adding back the EVEMS-clamped demand at 100% per NEC 625.42 — basis-consistent on both sides.
- **In-app panel-summary display fix.** `components/PanelSchedule.tsx` "Direct Demand Load" and "Demand Amps" cards now prefer `aggregatedLoad.totalDemandVA` whenever it's smaller than the panel-local `demandResult.totalDemandLoad_kVA × 1000` (i.e. the EVEMS clamp engaged) — not just when `downstreamPanelCount > 0` (which excluded leaf panels like the EV Sub-Panel). Inline `(NEC 625.42 EVEMS)` annotation when active.
- **`tests/calculations-extended.test.ts`:** 9 new regression tests:
  - 4× branch row VA — 12-charger EVEMS case (regression for the audit page-14 3,996 VA bug → expect 11,520 VA), non-EVEMS path (already correct, pinned), NEC 220.57(A) 7,200 VA minimum proof, EVEMS controller stays at 500 VA when no setpoint provided
  - 4× feeder/service clamp — clamps to explicit setpoint marker (preferred path), clamps via legacy panel-breaker proxy for backward compat, no clamp when no marker circuit (non-EVEMS EV panel), no clamp when already under setpoint
  - 1× MDP combined NEC 220.84 + 625.42 — full audit fixture (12 dwelling units + house + EVEMS-managed EV @ 12 chargers, explicit 47,952 VA setpoint marker); expected total demand 239–240 kVA / ≤1000 A (was over-clamping to 287–289 kVA / 1199 A in the first commit on this branch)
- **173/173 tests pass** (was 164 pre-C4, +9 new). `npm run build` exits 0 in 4.58s.
- **No DB migration.** Setpoint detection still uses a circuit-description heuristic stable against the autogen template; future follow-up F4: add explicit `panel.is_evems_managed` boolean + `panel.evems_setpoint_va` integer when the schema is next touched, then collapse the heuristic.

**Audit doc:** [`docs/AHJ_COMPLIANCE_AUDIT_2026-05-04.md`](AHJ_COMPLIANCE_AUDIT_2026-05-04.md). C4 marked ✅ RESOLVED. Sprint 1 complete. Open: C5 (PE seal workflow) — separate sprint; H-tier and M-tier findings — Sprint 2.

**Branch:** `fix/c4-evems-branch-circuit-math` / PR #19. Visual verification on Vercel preview pending before merge.
