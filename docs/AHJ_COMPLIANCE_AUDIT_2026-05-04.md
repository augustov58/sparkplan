...
**Sourcing caveat:** Orlando + Pompano + Davie are validated against official checklists (Pompano + Davie sourced 2026-05-12 — see [`docs/ahj-source-checklists/`](./ahj-source-checklists/)). Miami-Dade EV-specific PDF returns 404; sourced from Commercial Electrical PRG instead (Medium-High confidence). Hillsborough/Tampa has no dedicated EV PDF; residential lane undocumented (Medium confidence, gap noted in `SUMMARY.md`). **16 new candidate H-findings (H18–H33) surfaced** by the 4-AHJ research pass — see [`docs/ahj-source-checklists/SUMMARY.md`](./ahj-source-checklists/SUMMARY.md). Highest-severity: **H19 (HVHZ wind-anchoring documentation)** — cross-validated for any outdoor pedestal/bollard EVSE statewide. New cross-cutting architectural finding: **`building_type` (residential / multi-family / commercial) is a first-class manifest axis orthogonal to Sprint 2A's H17 contractor-exemption lane** — see [`sprints/sprint2c-ahj-manifests.md`](./sprints/sprint2c-ahj-manifests.md).
