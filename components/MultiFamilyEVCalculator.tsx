...
export const MultiFamilyEVCalculator: React.FC<MultiFamilyEVCalculatorProps> = ({ projectId, project, updateProject }) => {
  ...
  //
  // POSITIONING (Sprint 2C M6 fix-up, 2026-05-21): this effect MUST live
  // AFTER the `result = useMemo(...)` declaration above. Originally placed
  // before, but its dependency array `[result, buildingName]` is evaluated
  // synchronously during render, which threw a TDZ ReferenceError
  // ("Cannot access 'result' before initialization", minified as 'g') and
  // crashed the entire Tools tab via the FeatureErrorBoundary. Keep it
  // here next to handleAddEVInfrastructure (same invariant).
  useEffect(() => {
